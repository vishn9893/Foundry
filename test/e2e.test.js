...
test('registration, access control, workspace settings, docs, skills, and source flow', async () => {
  ...
  const skills = await (await fetch(`${base}/api/skills`, auth)).json(); assert.equal(skills.length, 0);
  const reviews = await (await fetch(`${base}/api/reviews`, auth)).json(); assert.equal(reviews.length, 1); assert.equal(reviews[0].review_status, 'pending');
  const review = await (await fetch(`${base}/api/reviews/${skill.id}`, { ...auth, method: 'PATCH', body: JSON.stringify({ status: 'approved' }) })).json(); assert.equal(review.status, 'approved');
  const published = await (await fetch(`${base}/api/skills`, auth)).json(); assert.equal(published.length, 1);
